...
def storeScore(user, userlink, sign, tableitem):
    try:
        ...
        print('db conn error!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    try:
        cur.execute("INSERT INTO score(user,userlink,signature) VALUES(%s,%s,%s)", (user.encode("utf-8"),userlink.encode("utf-8"),sign.encode("utf-8")))
        conn.commit()
    except:
        pass
    try:
...
def getVoters(ans_id, tableitem):
    ...
    while nextsource_url!=Zhihu:
        ...
        for each in nextdata['payload']:
            ...
            try:
                ...
                # print(txt3)
                continue
            storeScore(title, href, sign, tableitem)
        # 解析出指向下一组点赞者的资源
        nextsource_url = Zhihu + nextdata['paging']['next']

def getParams():
    input = open('zhihu.txt')
    for line in input:
        params = line.split(' ')
        print(params[0])
        print(params[1])
...
if __name__ == '__main__':
    ...
    for line in input:
        params = line.split(' ')
        agreeid = int(params[0])
        # print(params[1].split('\n')[0])
        t1 = threading.Thread(target=getVoters, args=(agreeid, params[1].split('\n')[0]))
        threads.append(t1)
    login()
    for t in threads:
        t.setDaemon(True)
